...
const ARTICLE_LEVEL_2_INLINE_TYPE = 'ARTICLE_LEVEL_2_TITLE_INLINE';
const ARTICLE_LEVEL_3_INLINE_TYPE = 'ARTICLE_LEVEL_3_TITLE_INLINE';
const BODY_TEXT_TYPE = 'BODY_TEXT';
const MIXED_PARAGRAPH_TYPE = 'MIXED_LEVEL_3_BODY';
const ARTICLE_PARAGRAPH_TYPE = 'ARTICLE_PARAGRAPH';
const ARTICLE_INLINE_TYPES = new Set([ARTICLE_LEVEL_2_INLINE_TYPE, ARTICLE_LEVEL_3_INLINE_TYPE]);
...
export function charUnitsToParagraphs(charUnits: CharUnit[]): ParagraphBlock[] {
  ...
  function flushCurrent() {
    ...
    const hasInlineLevel3 = current.segments.some((segment) => segment.type === INLINE_LEVEL_3_TYPE);
    const hasArticleInlineTitle = current.segments.some((segment) => ARTICLE_INLINE_TYPES.has(segment.type));
    if (
      (current.type === MIXED_PARAGRAPH_TYPE && hasInlineLevel3) ||
      (current.type === ARTICLE_PARAGRAPH_TYPE && hasArticleInlineTitle)
    ) {
...
function appendSegmentChar(segments: ParagraphSegment[], unit: CharUnit) {
  if (unit.type !== MIXED_PARAGRAPH_TYPE && unit.type !== ARTICLE_PARAGRAPH_TYPE) return;

  let type: ParagraphSegmentType = BODY_TEXT_TYPE;
  if (unit.type === MIXED_PARAGRAPH_TYPE && unit.segmentType === INLINE_LEVEL_3_TYPE) {
    type = INLINE_LEVEL_3_TYPE;
  } else if (unit.type === ARTICLE_PARAGRAPH_TYPE && unit.segmentType && ARTICLE_INLINE_TYPES.has(unit.segmentType)) {
    type = unit.segmentType;
  }
