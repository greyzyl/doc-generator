...
interface WorkflowSidebarProps {
  ...
  canRefineWithExamples: boolean;
  canGenerate: boolean;
  creatingDraft: boolean;
  parsing: boolean;
  parsingWithLLM: boolean;
  refiningWithExamples: boolean;
  savingParagraphs: boolean;
  generating: boolean;
  onCreateDraft: () => void;
  onParseByRule: () => void;
  onParseByLLM: () => void;
  onRefineByExamples: () => void;
  onGenerate: () => void;
  onUseSample: () => void;
...
export function WorkflowSidebar(props: WorkflowSidebarProps) {
  const [parseStrategy, setParseStrategy] = useState<ParseStrategy>('LLM');
  const isParsing = props.parsing || props.parsingWithLLM || props.refiningWithExamples;
    ...
    <aside className={`sidebar${props.collapsed ? ' is-collapsed' : ''}`}>
      ...
      <section className="panel-block quick-block">
        ...
        <div className="action-grid">
          ...
          <button
            type="button"
            onClick={props.onRefineByExamples}
            disabled={!props.canRefineWithExamples || isParsing || props.savingParagraphs || props.generating}
            className="btn green full-span"
          >
            {props.refiningWithExamples ? '重整中' : '用修改示例重整解析'}
          </button>
