...
export class DocumentApi {
  ...
  async createDraft(input: CreateDraftInput): Promise<CreateDraftResult> {
    ...
    formData.append('metadata', JSON.stringify(metadataForBackend(input.metadata)));
  ...
  async reclassifyByExamples(
    draftId: string,
    parseId: string,
    fileId: string,
    paragraphs: ParagraphBlock[],
    examples: ParagraphBlock[],
  ): Promise<ParseResult> {
    return this.client.json<ParseResult>(`/document-drafts/${encodeURIComponent(draftId)}/body/classify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async reclassifyByExamplesStream(
    draftId: string,
    parseId: string,
    fileId: string,
    paragraphs: ParagraphBlock[],
    ...
    const response = await fetch(this.client.endpoint(`/document-drafts/${encodeURIComponent(draftId)}/body/classify/stream`), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        parseId,
  ...
  async generateDocument(input: GenerateInput, draftId: string): Promise<GenerateResult> {
    return this.client.json<GenerateResult>(`/document-drafts/${encodeURIComponent(draftId)}/generate`, {
      ...
      body: JSON.stringify({
        ...
        metadata: metadataForBackend(input.metadata),
...
function metadataForBackend(metadata: Record<string, string>): Record<string, string> {
  const nextMetadata = { ...metadata };
  if (Object.prototype.hasOwnProperty.call(nextMetadata, 'printDate')) {
    nextMetadata.printDate = formatBackendPrintDate(nextMetadata.printDate);
  }
  return nextMetadata;
}

function formatBackendPrintDate(value: string | undefined): string {
  const rawValue = String(value || '').trim();
  const match = /^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(rawValue);
