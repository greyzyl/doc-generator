...
function segmentDisplay(segmentType?: string): { displayType: string; title: string } | null {
  if (segmentType === 'LEVEL_3_TITLE_INLINE') {
    return { displayType: 'LEVEL_3_TITLE', title: '段内三级标题' };
  }
  if (segmentType === 'ARTICLE_LEVEL_2_TITLE_INLINE') {
    return { displayType: 'CHAPTER_TITLE', title: '附件二级子标题' };
  }
  if (segmentType === 'ARTICLE_LEVEL_3_TITLE_INLINE') {
    return { displayType: 'LEVEL_2_TITLE', title: '附件三级子标题' };
  }
  return null;
}
    ...
    <main className="panel editor-panel" onClick={closeContextMenu}>
      ...
      <div ref={editorRef} onMouseUp={handleMouseUp} onContextMenuCapture={handleContextMenu} className="char-editor">
        ...
        {charUnits.map((unit, unitIndex) => {
          ...
          const segmentStyle = segmentDisplay(unit.segmentType);
          const displayType = segmentStyle?.displayType || unit.type;
          const title = segmentStyle?.title || getTypeLabel(unit.type);
