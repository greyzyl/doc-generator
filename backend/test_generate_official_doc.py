...
def main() -> None:
    ...
    config = load_json(Path("上行文.json"))
    assert_existing_paragraph_format(
        make_paragraph({"type": "ATTACHMENT_HEADER", "text": "附件：1.格式清单"}, config),
        left="0",
        right="0",
        first_line="640",
    ...
    assert_existing_paragraph_format(
        make_paragraph({"type": "ATTACHMENT_ITEM", "text": "附件内容第一段。"}, config),
        left="0",
        right="0",
        first_line="1600",
        first_line_chars="500",
        before="0",
        after="0",
        line="560",
        line_rule="exact",
