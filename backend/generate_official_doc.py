...
DOWNWARD_CLASSIFICATION_PROMPT = """\
你是下行文正文结构解析助手。请把用户提供的下行文正文解析成 JSON。

要求：
1. 只输出 JSON，不输出解释、不要输出 markdown、不要输出多余文本。
2. JSON 顶层必须是一个对象，并包含 bodyBlocks 数组。
3. 只解析正文主体结构。若输入来自完整格式样稿，遇到版头、发文字号、版记抄送、印发机关和印发日期等固定版头版记内容，不要输出到 bodyBlocks。
   例如“中铁二十四局集团有限公司文件”“二十四局办发〔2025〕220号”“抄送：存档。”“……办公室……印发”等不属于正文主体。
4. 输出的每个 bodyBlocks 元素必须逐字保留对应正文行原文，不要合并、拆分、改写、润色原文。
5. type 只能使用：
   MAIN_TITLE, RECIPIENT, BODY_PARAGRAPH,
   SIGNING_COMPANY, SIGNING_DATE,
   APPENDIX_TITLE, CHAPTER_TITLE, ARTICLE_PARAGRAPH,
   ATTACHMENT_HEADER, ATTACHMENT_ITEM, CONTACT_INFO。
   不要输出 SUB_TITLE、LEVEL_1_TITLE、LEVEL_2_TITLE、LEVEL_3_TITLE、MIXED_LEVEL_3_BODY。
6. 下行文主标题通常是“关于……的通知/决定/通报/批复”等，可能被 Word 拆成连续多行居中标题。
   多行标题不要合并；每一行都识别为 MAIN_TITLE。
7. RECIPIENT 是主送机关，通常位于标题之后并以冒号结尾，例如“所属各单位，总部各部门:”“各单位：”。
8. 标题和主送机关后的发布、印发、执行要求等导语识别为 BODY_PARAGRAPH，例如“现将《……办法》印发给你们，请遵照执行。”。
9. 导语之后的发文单位/联合发文单位落款识别为 SIGNING_COMPANY；紧随其后的日期识别为 SIGNING_DATE。
   “集团公司           集团公司党委”这类同一行多单位落款仍是 SIGNING_COMPANY。
10. 如果落款日期之后进入被印发的制度、办法、细则、规定等附件正文：
   - 制度/办法名称识别为 APPENDIX_TITLE，例如“中铁二十四局集团有限公司公文处理办法”。
   - “第一章  总  则”“第二章  公文种类和形式”这类章标题识别为 CHAPTER_TITLE。
   - “第一条  ……”“第二条  ……”这类条文识别为 ARTICLE_PARAGRAPH，并使用 segments 保留段内标题：
     第一个 segment type=ARTICLE_LEVEL_2_TITLE_INLINE，text 只放“第一条”这类条号；
     第二个 segment type=BODY_TEXT，text 放该段剩余原文，两个 segment 拼接后必须等于原文。
   - 条文被 Word 断行后，续行虽然没有“第X条”开头，也识别为 ARTICLE_PARAGRAPH；续行没有段内标题时直接使用 text，不要硬拆 segments。
   - “（一）决议。……”“（二）决定。……”“（四）意见。……”这类条文内部列项识别为 ARTICLE_PARAGRAPH，并使用 segments 保留段内标题：
     第一个 segment type=ARTICLE_LEVEL_3_TITLE_INLINE，text 放“（二）决定。”这类括号序号加短标题和标题结束标点；
     第二个 segment type=BODY_TEXT，text 放该段剩余原文。
11. 附件清单识别：
   - “附件：1．……”识别为 ATTACHMENT_HEADER。
   - 后续“2．……”“3．……”等同属附件清单的行识别为 ATTACHMENT_ITEM。
12. 联系人/联系电话括号内容识别为 CONTACT_INFO。
13. 置信度低时保守识别为 BODY_PARAGRAPH；不要为了套旧标题层级而误判章、条、款、项结构。

输出示例：
{
  "bodyBlocks": [
    {"type": "MAIN_TITLE", "text": "关于修订《中铁二十四局集团有限公司"},
    {"type": "MAIN_TITLE", "text": "公文处理办法》的通知"},
    {"type": "RECIPIENT", "text": "所属各单位，总部各部门:"},
    {"type": "BODY_PARAGRAPH", "text": "现将《中铁二十四局集团有限公司公文处理办法》印发给你们，请遵照执行。"},
    {"type": "SIGNING_COMPANY", "text": "集团公司           集团公司党委"},
    {"type": "SIGNING_DATE", "text": "2025年6月16日"},
    {"type": "APPENDIX_TITLE", "text": "中铁二十四局集团有限公司公文处理办法"},
    {"type": "CHAPTER_TITLE", "text": "第一章  总  则"},
    {"type": "ARTICLE_PARAGRAPH", "segments": [
      {"type": "ARTICLE_LEVEL_2_TITLE_INLINE", "text": "第一条"},
      {"type": "BODY_TEXT", "text": "  为加强中铁二十四局集团有限公司公文处理工作，制定本办法。"}
    ]},
    {"type": "ARTICLE_PARAGRAPH", "segments": [
      {"type": "ARTICLE_LEVEL_3_TITLE_INLINE", "text": "（一）决议。"},
      {"type": "BODY_TEXT", "text": "适用于重要会议讨论通过的重大决策事项。"}
    ]},
    {"type": "ATTACHMENT_HEADER", "text": "附件：1．中铁二十四局集团有限公司公文格式及表单式样"},
    {"type": "ATTACHMENT_ITEM", "text": "2．中铁二十四局集团有限公司及所属各单位规范化简称"}
  ]
}
"""

STREAMING_OUTPUT_SUFFIX = """

流式输出补充要求：
1. 本次输入会以 JSON 数组给出，每项包含 index 和 text。
2. 请按输入 index 顺序输出 JSON Lines，每一行是一个独立 JSON 对象。
3. 每行对象必须包含 index 和 type；普通段落包含 text；MIXED_LEVEL_3_BODY 和带段内标题的 ARTICLE_PARAGRAPH 包含 segments。
...
EXAMPLE_GUIDED_SUFFIX = """

示例引导补充要求：
1. 用户会提供 correctedExamples 和 targetLines 两组数据。
2. correctedExamples 是用户已人工修正过的解析结果；请把它们当作本次文档的标注示例和偏好规则。
3. targetLines 是需要重新解析的目标正文；必须只按 targetLines 的顺序输出 bodyBlocks。
4. 不要把 correctedExamples 中的文本复制到无关 targetLines；示例只用于学习同类段落的 type 和 segments 拆分习惯。
5. 若 correctedExamples 与通用规则冲突，优先遵循用户示例体现出的分类偏好。
6. 输出仍然必须是 JSON 对象，且只包含 bodyBlocks，不要输出解释。
"""

STREAMING_EXAMPLE_GUIDED_SUFFIX = """

示例引导流式输出补充要求：
1. 用户会提供 correctedExamples 和 targetLines 两组数据。
2. correctedExamples 是用户已人工修正过的解析结果；请把它们当作本次文档的标注示例和偏好规则。
3. targetLines 是需要重新解析的目标正文；必须只按 targetLines 的 index 顺序输出。
4. 不要把 correctedExamples 中的文本复制到无关 targetLines；示例只用于学习同类段落的 type 和 segments 拆分习惯。
5. 如果 correctedExamples 与通用规则冲突，优先遵循用户示例体现出的分类偏好。
6. 请输出 JSON Lines，每一行是一个独立 JSON 对象。
7. 每行对象必须包含 index 和 type；普通段落包含 text；MIXED_LEVEL_3_BODY 和带段内标题的 ARTICLE_PARAGRAPH 包含 segments。
8. 不要输出 JSON 数组，不要输出逗号分隔，不要输出 markdown，不要输出解释。
"""

STREAMING_CLASSIFICATION_PROMPT = SEMANTIC_CLASSIFICATION_PROMPT + STREAMING_OUTPUT_SUFFIX
EXAMPLE_GUIDED_CLASSIFICATION_PROMPT = SEMANTIC_CLASSIFICATION_PROMPT + EXAMPLE_GUIDED_SUFFIX
STREAMING_EXAMPLE_GUIDED_CLASSIFICATION_PROMPT = SEMANTIC_CLASSIFICATION_PROMPT + STREAMING_EXAMPLE_GUIDED_SUFFIX
DOWNWARD_STREAMING_CLASSIFICATION_PROMPT = DOWNWARD_CLASSIFICATION_PROMPT + STREAMING_OUTPUT_SUFFIX
DOWNWARD_EXAMPLE_GUIDED_CLASSIFICATION_PROMPT = DOWNWARD_CLASSIFICATION_PROMPT + EXAMPLE_GUIDED_SUFFIX
DOWNWARD_STREAMING_EXAMPLE_GUIDED_CLASSIFICATION_PROMPT = DOWNWARD_CLASSIFICATION_PROMPT + STREAMING_EXAMPLE_GUIDED_SUFFIX

NO_LEVEL3_CLASSIFICATION_NOTE = """

特定模板补充要求：
1. 本次模板不使用三级标题。
2. “1.”“1．”“1、”等数字编号开头的段落一律识别为 BODY_PARAGRAPH。
3. 不要输出 LEVEL_3_TITLE、MIXED_LEVEL_3_BODY 或 LEVEL_3_TITLE_INLINE。
"""
...
ALLOWED_SEGMENT_TYPES = {
    "LEVEL_3_TITLE_INLINE",
    "ARTICLE_LEVEL_2_TITLE_INLINE",
    "ARTICLE_LEVEL_3_TITLE_INLINE",
    "BODY_TEXT",
}

# 定义一组正则规则，后面用来判断一行正文属于什么类型
CN_NUM = "一二三四五六七八九十" #各级标题
ARTICLE_CN_NUM = "一二三四五六七八九十百千万零〇两"
...
ARTICLE_LEVEL_2_RE = re.compile(rf"^(第[{ARTICLE_CN_NUM}]+条)(.*)$")
ARTICLE_LEVEL_3_RE = re.compile(rf"^(（[{ARTICLE_CN_NUM}]+）)(.*)$")
...
其他：BODY_PARAGRAPH
"""
def classify_plain_text(text: str, *, disable_level3: bool = False) -> list[dict[str, Any]]:
    ...
    for index, line in enumerate(lines):
        ...
        elif not disable_level3 and LEVEL_3_RE.match(line):
...
def classify_plain_text_with_llm(
    ...
    *,
    ...
    raw_response = call_chat_completion(
        ...
        model=model,
        messages=[
            {"role": "system", "content": classification_prompt(prompt_profile, "plain", disable_level3=disable_level3)},
            {"role": "user", "content": "请解析以下正文：\n\n" + text},
        ],
        timeout=timeout,
    )
    blocks = normalize_body_blocks(parse_json_payload(raw_response))
    return coerce_level3_to_body_blocks(blocks) if should_coerce_level3(prompt_profile, disable_level3) else blocks


def classify_plain_text_with_llm_examples(
    text: str,
    *,
    ...
    normalized_examples = normalize_body_blocks(examples)
    target_lines = [
        {"index": index, "text": line}
        for index, line in enumerate(lines, start=1)
    ]
    prompt_payload = {
        "correctedExamples": normalized_examples,
    ...
    raw_response = call_chat_completion(
        ...
        model=model,
        messages=[
            {"role": "system", "content": classification_prompt(prompt_profile, "example", disable_level3=disable_level3)},
            {
                "role": "user",
                "content": "请基于用户修正示例重新解析目标正文：\n\n"
...
def classify_plain_text_with_llm_stream(
    ...
    *,
    ...
    for delta in call_chat_completion_stream(
        ...
        model=model,
        messages=[
            {"role": "system", "content": classification_prompt(prompt_profile, "stream", disable_level3=disable_level3)},
        ...
        while "\n" in buffer:
            ...
            if index in yielded:
                continue
            yielded.add(index)
            if should_coerce_level3(prompt_profile, disable_level3):
                block = coerce_level3_to_body_blocks([block])[0]
            yield index, block
...
def classify_plain_text_with_llm_examples_stream(
    text: str,
    *,
    examples: list[dict[str, Any]],
    model: str,
    ...
    for delta in call_chat_completion_stream(
        ...
        messages=[
            {
                "role": "system",
                "content": classification_prompt(prompt_profile, "stream_example", disable_level3=disable_level3),
            },
            {
                "role": "user",
                "content": "请按 JSON Lines 基于用户修正示例重新解析目标正文：\n\n"
                + json.dumps(prompt_payload, ensure_ascii=False),
            },
        ],
        timeout=timeout,
    ...
    if len(yielded) == len(target_lines):
        return
...
def classification_prompt(prompt_profile: str, mode: str, *, disable_level3: bool = False) -> str:
    profile = (prompt_profile or "DEFAULT").upper()
    if profile == "DOWNWARD":
        prompts = {
            "plain": DOWNWARD_CLASSIFICATION_PROMPT,
            "stream": DOWNWARD_STREAMING_CLASSIFICATION_PROMPT,
            "example": DOWNWARD_EXAMPLE_GUIDED_CLASSIFICATION_PROMPT,
            "stream_example": DOWNWARD_STREAMING_EXAMPLE_GUIDED_CLASSIFICATION_PROMPT,
        }
    else:
        prompts = {
            "plain": SEMANTIC_CLASSIFICATION_PROMPT,
            "stream": STREAMING_CLASSIFICATION_PROMPT,
            "example": EXAMPLE_GUIDED_CLASSIFICATION_PROMPT,
            "stream_example": STREAMING_EXAMPLE_GUIDED_CLASSIFICATION_PROMPT,
        }
    prompt = prompts.get(mode, prompts["plain"])
    if disable_level3:
        prompt += NO_LEVEL3_CLASSIFICATION_NOTE
    return prompt


def should_coerce_level3(prompt_profile: str, disable_level3: bool) -> bool:
    return disable_level3 or (prompt_profile or "").upper() == "DOWNWARD"
...
def coerce_level3_to_body_blocks(blocks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    coerced: list[dict[str, Any]] = []
    for block in blocks:
        block_type = str(block.get("type") or "BODY_PARAGRAPH")
        if block_type in {"LEVEL_3_TITLE", "MIXED_LEVEL_3_BODY"}:
            coerced.append({"type": "BODY_PARAGRAPH", "text": block_plain_text(block)})
        else:
...
def split_article_paragraph_segments(line: str) -> list[dict[str, str]] | None:
    text = str(line or "")
    level2_match = ARTICLE_LEVEL_2_RE.match(text)
    if level2_match:
        return [
            {"type": "ARTICLE_LEVEL_2_TITLE_INLINE", "text": level2_match.group(1)},
            {"type": "BODY_TEXT", "text": level2_match.group(2)},
        ]

    level3_match = ARTICLE_LEVEL_3_RE.match(text)
    if not level3_match:
        return None

    marker = level3_match.group(1)
    rest = level3_match.group(2)
    title_end = re.search(r"[。；;:：]", rest)
    if not title_end:
        return None

    title_body = rest[: title_end.end()]
    if len(title_body) > 16:
        return None

    return [
        {"type": "ARTICLE_LEVEL_3_TITLE_INLINE", "text": marker + title_body},
        {"type": "BODY_TEXT", "text": rest[title_end.end() :]},
    ]
...
def make_p_pr(style: dict[str, Any]) -> ET.Element:
    ...
    if style.get("pageBreakBefore"):
        ET.SubElement(p_pr, qn("pageBreakBefore"))
...
def estimate_rendered_text_width_twips(text: str, run_style: dict[str, Any]) -> float:
    font_size_pt = float(run_style.get("fontSizePt") or 16)
    width_units = sum(estimate_character_width_unit(char) for char in str(text or ""))
    return width_units * font_size_pt * 20


def estimate_character_width_unit(char: str) -> float:
    if char == "\t":
        return 4.0
    if char.isspace():
        return 0.5
...
def block_display_text(block: dict[str, Any]) -> str:
    segments = block.get("segments")
    if isinstance(segments, list):
...
def text_width_for_block(block: dict[str, Any], config: dict[str, Any]) -> float:
    _p_style_key, r_style_key = style_for_block(str(block.get("type") or "BODY_PARAGRAPH"), config)
    run_style = config["runStyles"].get(r_style_key, config["runStyles"].get("BODY_TEXT", {}))
    return estimate_rendered_text_width_twips(block_display_text(block), run_style)


def paragraph_style_for_block(block: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    p_style_key, _r_style_key = style_for_block(str(block.get("type") or "BODY_PARAGRAPH"), config)
    return config["paragraphStyles"].get(p_style_key, {})
...
def style_for_block(block_type: str, config: dict[str, Any]) -> tuple[str, str]:
    ...
    if block_type == "APPENDIX_TITLE":
        return "APPENDIX_TITLE", "MAIN_TITLE_TEXT"
    if block_type == "CHAPTER_TITLE":
        return "CHAPTER_TITLE", "CHAPTER_TITLE_TEXT"
    if block_type == "ARTICLE_PARAGRAPH":
        return "ARTICLE_PARAGRAPH", "BODY_TEXT"
...
def style_for_segment(segment_type: str) -> str:
    ...
    if segment_type == "ARTICLE_LEVEL_2_TITLE_INLINE":
        return "ARTICLE_LEVEL_2_TITLE_INLINE"
    if segment_type == "ARTICLE_LEVEL_3_TITLE_INLINE":
        return "ARTICLE_LEVEL_3_TITLE_INLINE"
    return "BODY_TEXT"
...
def make_paragraph(
    block: dict[str, Any],
    config: dict[str, Any],
    paragraph_style_overrides: dict[str, Any] | None = None,
) -> ET.Element:
    block_type = block.get("type", "BODY_PARAGRAPH")
    p_style_key, r_style_key = style_for_block(block_type, config)
    p = ET.Element(qn("p"))
    paragraph_style = {
        **config["paragraphStyles"].get(p_style_key, {}),
        **(paragraph_style_overrides or {}),
    }
    p.append(make_p_pr(paragraph_style))

    segments = block.get("segments")
    if not segments and block_type == "ARTICLE_PARAGRAPH":
        segments = split_article_paragraph_segments(str(block.get("text") or ""))
    if segments:
...
def insert_body_blocks(root: ET.Element, config: dict[str, Any], blocks: list[dict[str, Any]]) -> None:
    ...
    for index, child in enumerate(children):
        if child.tag == qn("p") and anchor in paragraph_text(child):
            body.remove(child)
            signing_company_metrics: dict[str, float] | None = None
            for offset, block in enumerate(blocks):
                block_type = str(block.get("type") or "BODY_PARAGRAPH")
                paragraph_style_overrides = None

                if block_type == "SIGNING_COMPANY":
                    company_style = paragraph_style_for_block(block, config)
                    signing_company_metrics = {
                        "textWidth": text_width_for_block(block, config),
                        "rightIndent": float(company_style.get("rightIndent") or 0),
                    }
                elif block_type == "SIGNING_DATE" and signing_company_metrics:
                    date_width = text_width_for_block(block, config)
                    right_indent = signing_company_metrics["rightIndent"] + (
                        signing_company_metrics["textWidth"] - date_width
                    ) / 2
                    paragraph_style_overrides = {"rightIndent": round(right_indent)}
                    signing_company_metrics = None

                body.insert(index + offset, make_paragraph(block, config, paragraph_style_overrides))
            return
